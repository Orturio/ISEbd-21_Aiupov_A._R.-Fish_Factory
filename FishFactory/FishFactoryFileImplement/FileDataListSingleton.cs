    ...
    public class FileDataListSingleton
    {
        ...
        private List<Warehouse> LoadWarehouses()
        {
            ...
            if (File.Exists(WarehouseFileName))
            {
                ...
                foreach (var elem in xElements)
                {
                    var warehouseCom = new Dictionary<int, int>();
                    foreach (var component in elem.Element("WarehouseComponents").Elements("WarehouseComponent").ToList())
                    {
                        warehouseCom.Add(Convert.ToInt32(component.Element("Key").Value),
...
Convert.ToInt32(component.Element("Value").Value));
        ...
        private void SaveWarehouses()
        {
            if (Warehouses != null)
            {
                ...
                {
                    var warehElement = new XElement("WarehouseComponents");

                    foreach (var component in warehouse.WarehouseComponents)
                    {
                        warehElement.Add(new XElement("CannedComponents",
